        ...
        @auth
            @if (auth()->user()->id === $user->id && $user->role === 'Guide')
                <div class="min-w-screen min-h-screen bg-white flex items-center p-5 lg:p-10 overflow-hidden relative">
                    ...
                    <h1 class="text-center font-bold text-4xl pb-10">Details of the offer</h1>
                            ...
                            <div class="w-full md:w-1/2 px-10">
                                <div class="mb-10">
                                   <!-- <h1 class="font-bold uppercase text-2xl mb-5">Mens's Ragged <br>Waterproof Jacket</h1>
                                    ...
                                    <p><span class="font-semibold text-xl">Offer from : </span><span class="text-xl">{{ $booking->user->firstname}}</span></p>
                                    <p><span class="font-semibold text-xl">Received on : </span><span class="text-xl">{{ Carbon\Carbon::parse($booking->booked_at)->format('d/m/Y') }}</span></p>
                                    ...
                                    <div class="inline-block align-bottom">
                                        <div>
                                            @if ($booking->status_demand === 'pending')
                                                <form action="{{ route('accept_offer', $booking->id) }}" method="POST" class="inline-block">
                                                    @csrf
                                                ...
                                                <form action="{{ route('refuse_offer', $booking->id) }}" method="POST" class="inline-block">
                                                    @csrf
                                            ...
                                            @elseif ($booking->status_offer === 'waiting for paiement')
                                                <p>Status : <span class="text-blue-400 font-bold text-lg">{{ $booking->status_offer }}</span></p>
                                            @elseif ($booking->status_offer === 'booked')
                                                <p>Status : <span class="text-blue-400 font-bold text-lg">{{ $booking->status_offer }}</span></p>
                                            @elseif ($booking->status_offer === 'refused')
                                                <p>Status : <span class="text-blue-400 font-bold text-lg">{{ $booking->status_offer }}</span></p>
                                            @endif
                                        </div> 
                                    </div>
