  ...
  <div id='menu' class="w-full h-screen flex-grow lg:flex lg:items-center sm:h-auto lg:w-auto hidden lg:block">
        @auth
            <div class="text-xl lg:text-lg text-white text-center pl-7 lg:flex-grow lg:pl-36 {{ request()->is('dashboard') ? 'active' : ''}}">
                ...
                @endif
                <a href="{{ route('add_article_form') }}" class="py-4 lg:py-0 lg:mb-2.5 inline-block mr-8 mr-16 lg:mr-8 lg:ml-3 transform hover:scale-110 motion-reduce:transform-none">
        ...
        <div class="top-0 px-6 py-4 sm:inline-block">
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    <button type="sumbit" class="inline-block leading-none lg:mt-0 px-7 py-2 lg:px-4 text-xl lg:text-lg align-middle font-semibold tracking-wider border-2 text-white border-gray-darker rounded-full focus:ring-2 focus:ring-sun cursor-pointer hover:shadow-lg hover:text-sun">
                        Log out
                    </button>
                    @csrf
                </form>
            @else
                ...
                <a href="{{ route('login') }}" class="inline-block mr-2 text-xl lg:text-lg px-4 py-2 leading-none border rounded text-white border-white hover:border-sun hover:text-sun mt-4 lg:mt-0">
                    Log in
                </a>
                @if (Route::has('register'))
                    <!--<a href="{{ route('register') }}" class="ml-4 text-sm text-gray-700 underline">Register</a>-->
                    <a href="{{ route('register') }}" class="inline-block ml-2 text-xl lg:text-lg px-4 py-2 leading-none border rounded text-white border-white hover:border-sun hover:text-sun mt-4 lg:mt-0">
                        Register
                    </a>
                @endif
            @endauth
