    ...
    <main class="bg-pink-200">
        @auth
            @if(auth()->user()->id === $author->id)
                <form method="POST" action="{{ route('save_article', $article->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')   
                    <div>
                        <label for="categories">Choose related categories</label> <!-- multiple -->
                ...
                <a href="{{ route('show_article', $article->id) }}" class="mt-6 px-7 py-2 text-xl md:mt-0 lg:text-base align-middle font-semibold tracking-wider border-2 text-white border-first bg-first rounded-full focus:ring-2 focus:ring-sun cursor-pointer hover:shadow-lg hover:text-sun">
                    Back
                </a>
            @endif
        @endauth
